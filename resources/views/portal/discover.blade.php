...
@section('content')
    ...
    <div class="portal-shell" x-data="{
        ...
        <header class="p-header" role="banner">
            <div class="p-header-inner">
                ...
                <a href="/" class="p-brand" aria-label="Ostrich — home">
                    <img src="/img/fayiloli-icon.svg" class="p-brand-icon" alt="" aria-hidden="true" width="26" height="26">
                    <span class="p-brand-name">Ostrich </span>
        ...
        <section class="p-hero" aria-labelledby="portal-heading">
            ...
            <div class="p-hero-body">
                ...
                <div class="p-search-wrap" role="search">
                    ...
                    <input type="search" id="portalSearch" x-ref="searchInput" x-model.debounce.150ms="search"
                        class="p-search-input" placeholder="Type an organisation name or type…" autocomplete="off"
                        spellcheck="false" aria-describedby="portalCount">
                    <button type="button" class="p-search-clear" x-show="search.length > 0" x-cloak
                        @click="search = ''; $refs.searchInput.focus()" aria-label="Clear search">
                        <i class="fas fa-xmark" aria-hidden="true"></i>
                    </button>
                </div>

                {{-- Type filter pills (hidden when only one type exists) --}}
                <div class="p-type-filters" role="group" aria-label="Filter by organisation type"
                    x-show="uniqueTypes.length > 1" x-cloak>
                    {{-- "All" pill --}}
                    <button type="button" class="p-pill" :class="{ 'p-pill--active': activeType === 'all' }"
                        @click="activeType = 'all'" :aria-pressed="(activeType === 'all').toString()">
                        All
                    ...
                    <template x-for="type in uniqueTypes" :key="type">
                        <button type="button" class="p-pill" :class="{ 'p-pill--type-active': activeType === type }" :style="activeType === type
                                    ? (() => { const c = orgs.find(o => o.type_key === type)?.color || '#7c3aed'; return 'border-color:'+c+';color:'+c+';background:'+c+'1a'; })()
                                    : ''" @click="selectType(type)" :aria-pressed="(activeType === type).toString()">
        ...
        <main class="p-main">
            ...
            @if ($totalCount > 0)
                ...
                {{-- Stats strip --}}
                <div style="display:flex;align-items:center;gap:0.5rem;font-size:0.78rem;color:#64748b;margin-bottom:1rem;font-weight:500">
                    <i class="fas fa-building-columns" style="font-size:0.7rem;opacity:0.6" aria-hidden="true"></i>
                    <span>{{ $totalCount }} {{ $totalCount === 1 ? 'organisation' : 'organisations' }}</span>
                    <span style="color:#cbd5e1">&middot;</span>
                    <span>Filter by type <i class="fas fa-arrow-right" style="font-size:0.65rem" aria-hidden="true"></i></span>
                </div>
                ...
                <div class="p-grid" role="list" aria-label="Available organisations">
                    ...
                    <template x-for="org in filtered" :key="org.id">
                        <article class="p-card" role="listitem"
                            tabindex="0"
                            @keydown.enter="window.location.href = org.url"
                            :style="'border-left: 4px solid ' + org.color"
                            x-transition:enter="p-card-enter"
                            x-transition:enter-start="p-card-enter-from" x-transition:enter-end="p-card-enter-to">
                            <div class="p-card-top">
                                ...
                                <div class="p-card-meta">
                                    ...
                                    <span class="p-card-badge"
                                        ...
                                        x-text="org.type"></span>
                                </div>
                            </div>

                            <a class="p-card-btn" :href="org.url" :style="'background:' + org.color"
                                :aria-label="'Sign in to ' + org.name" x-data="{ going: false }" @click="going = true"
                ...
                <div class="p-empty" x-show="filtered.length === 0" x-cloak x-transition role="status" aria-live="polite"
                    aria-atomic="true">
        ...
        <footer class="p-footer" role="contentinfo">
            ...
            <p>&copy; {{ date('Y') }} NectarMetrics Solutions Limited &middot; Ostrich</p>
        </footer>
    ...
    <style>
        ...
        /* pills are exempt from 44px as inline controls */
        .p-card-btn {
            min-height: 40px;
        }
        ...
        .p-search-input::-webkit-search-cancel-button {
            -webkit-appearance: none;
        }
