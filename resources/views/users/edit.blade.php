...
<div class="container py-4" style="max-width:760px">
    ...
    <form action="{{ route('users.update', $user) }}" method="POST" class="card border-0 shadow-sm p-4" novalidate>
        @csrf
        @method('PUT')
                ...
                <select id="roles" name="roles[]" class="form-select" multiple size="5">
                    @foreach($roles as $role)
                        <option value="{{ $role->name }}"
                                {{ (old('roles') !== null ? in_array($role->name, old('roles', [])) : $user->hasRole($role->name)) ? 'selected' : '' }}>
                            {{ ucfirst($role->name) }}
                        </option>
                    @endforeach
                </select>
                ...
                <select id="permissions" name="permissions[]" class="form-select" multiple size="5">
                    @foreach($permissions as $permission)
                        <option value="{{ $permission->name }}"
                                {{ (old('permissions') !== null ? in_array($permission->name, old('permissions', [])) : $user->hasPermissionTo($permission->name)) ? 'selected' : '' }}>
                            {{ $permission->name }}
                        </option>
                    @endforeach
                </select>
