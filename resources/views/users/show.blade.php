            ...
            <div class="col-lg-4">
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-body text-center pt-4">
                        ...
                        <p class="text-muted small mb-2">{{ '@' . $user->user_name }}</p>
                    ...
                    <div class="card-footer bg-transparent small text-muted">
                        <dl class="row mb-0 g-0" style="font-size:0.78rem">
                                ...
                                <dd class="col-7">
                                    @can('view', $user->supervisor)
                                        <a href="{{ route('users.show', $user->supervisor) }}"
                                           class="text-decoration-none">{{ $user->supervisor->name }}</a>
                                    @else
                            ...
                            @if($user->is_locked && $user->locked_at)
                                <dt class="col-5 text-danger">Locked</dt>
                                <dd class="col-7 text-danger">{{ $user->locked_at->format('d M Y H:i') }}</dd>
                            @endif
                            <dt class="col-5">Failed logins</dt>
                            <dd class="col-7">
                                <span class="{{ ($user->failed_login_attempts ?? 0) >= 3 ? 'text-warning fw-semibold' : '' }}">
                ...
                <div class="card border-0 shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        @forelse($user->roles->sortBy('name') as $role)
                            <div class="d-flex align-items-center gap-3 mb-2 pb-2 border-bottom last:border-0">
                                <x-ts-badge :text="ucfirst($role->name)" color="violet" />
                                <span class="text-muted small">
                                    {{ $role->permissions_count ?? $role->permissions->count() }} permission{{ ($role->permissions->count() ?? 1) !== 1 ? 's' : '' }}
                                </span>
                                @can('manage roles')
                                    <a href="{{ route('roles.show', $role) }}"
                                       class="ms-auto btn btn-link btn-sm text-muted p-0"
                                       title="View role">
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-transparent py-3">
                        <h6 class="fw-semibold mb-0">
                            ...
                            Effective Permissions
                            <span class="badge bg-secondary ms-2" style="font-size:0.7rem">{{ $allPermissions->count() }}</span>
                        </h6>
                        <p class="text-muted small mb-0 mt-1">All permissions this user has (via roles + direct assignments)</p>
                    </div>
                    <div class="card-body">
                        @php
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex flex-wrap gap-1">
                                    @foreach($perms as $perm)
                                        ...
                                        @endphp
                                        <span class="badge rounded-pill {{ $isDirect ? 'bg-primary' : 'bg-light text-dark border' }}"
                                              title="{{ $isDirect ? 'Directly assigned' : 'Via role' }}"
                        ...
                        @if($directPerms->count() > 0)
                            <p class="text-muted mt-2 mb-0" style="font-size:0.72rem">
                                <i class="fas fa-star me-1 text-primary" aria-hidden="true"></i>
