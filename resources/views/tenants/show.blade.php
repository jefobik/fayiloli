...
@section('content')
                    ...
                    @can('delete', $tenant)
                        <div class="card-footer bg-white border-top pt-3">
                            <button type="button" class="btn btn-sm btn-outline-danger"
                                ...
                                data-delete-url="{{ route('tenants.destroy', $tenant) }}" onclick="confirmDeleteTenant(this)"
                        ...
                        @if (count($transitions))
                            ...
                            <div class="d-flex flex-column gap-2">
                                @foreach ($transitions as $t)
                                    <button type="button" class="btn btn-sm {{ $t['btnClass'] }} text-start"
                                        data-target-status="{{ $t['target']->value }}" data-action-label="{{ $t['action'] }}"
                                        data-tenant-name="{{ $tenant->organization_name }}"
                    ...
                    <div class="card-body">
                                ...
                                @foreach ($tenant->domains as $domain)
                                    <div class="d-flex gap-2">
                                        <a href="http://{{ $domain->domain }}" target="_blank" rel="noopener"
                                            ...
                                            title="Visit standard login page">
                                            <i class="fa-solid fa-globe me-1" aria-hidden="true"></i>
                                            {{ $domain->domain }}
                                        </a>
                                        @if($loop->first)
                                            <a href="{{ route('tenants.impersonate', $tenant) }}" target="_blank"
                                                class="btn btn-sm btn-primary flex-shrink-0 text-nowrap"
                                                title="Securely enter workspace without a password">
                                                <i class="fa-solid fa-user-shield me-1"></i> Enter
                                            </a>
                                        @endif
                                    </div>
                                @endforeach
                        ...
                        <div class="d-flex flex-wrap gap-1">
                            @foreach ($tenantModules as $module)
                                ...
                                <span
                                    class="badge rounded-pill {{ $enabled ? 'bg-success' : 'bg-light text-secondary border' }}"
                                ...
                                @forelse ($tenant->domains as $domain)
                                    <div
                                        ...
                                        <button type="button" class="btn btn-sm btn-outline-danger border-0"
                                            ...
                                            data-domain-id="{{ $domain->id }}" onclick="confirmRemoveDomain(this)">
    ...
    <div class="modal fade" id="transitionModal" tabindex="-1" aria-labelledby="transitionModalLabel" aria-modal="true"
        role="dialog">
    ...
    <div class="modal fade" id="deleteTenantModal" tabindex="-1" aria-labelledby="deleteTenantModalLabel" aria-modal="true"
        role="dialog">
        <div class="modal-dialog modal-dialog-centered">
    ...
    {{-- ── Remove Domain Modal ─────────────────────────────────────────────────── --}}
    <div class="modal fade" id="removeDomainModal" tabindex="-1" aria-labelledby="removeDomainModalLabel" aria-modal="true"
        role="dialog">
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-body pt-2">
                    <p class="mb-0 small">Remove <span class="fw-semibold font-monospace" id="removeDomainLabel"></span>?
                    </p>
                </div>
