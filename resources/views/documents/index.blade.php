...
--}}
<div class="edms-browser flex flex-col overflow-hidden bg-slate-50 dark:bg-slate-950
            -m-4 sm:-m-6 lg:-m-8"
    ...
    <div x-show="isDragging"
        ...
        <div class="absolute inset-0 bg-indigo-600/10 backdrop-blur-sm border-4 border-dashed border-indigo-400 dark:border-indigo-500 rounded-none"></div>
        <div class="relative bg-white dark:bg-slate-900 rounded-2xl px-10 py-8 shadow-2xl text-center border border-indigo-200 dark:border-indigo-800">
            <div class="flex items-center justify-center w-16 h-16 mx-auto mb-4 rounded-full bg-indigo-50 dark:bg-indigo-900/40 text-indigo-500">
    ...
    <div class="flex items-center gap-1.5 sm:gap-2 flex-wrap px-3 sm:px-4 py-2 border-b
                border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0
                shadow-sm" role="toolbar" aria-label="Document actions">

        @can('create documents')
        {{-- Primary: Upload Files --}}
        <button type="button"
                class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold
        ...
        <button type="button"
            ...
            <i class="fas fa-folder-open text-slate-400" aria-hidden="true"></i>
        ...
        <div class="hidden sm:block h-5 w-px bg-slate-200 dark:bg-slate-700 mx-0.5" role="separator" aria-hidden="true"></div>
        ...
        <button type="button"
                ...
                onclick="requestDocument()" aria-label="Request a document" aria-haspopup="dialog">
            <i class="fas fa-arrow-down-tray text-slate-400" aria-hidden="true"></i>
        ...
        <button type="button"
                class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold
                       ...
                       hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors
                       focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1"
                data-bs-toggle="modal" data-bs-target="#shareDocumentModal"
                onclick="shareDocument()" aria-label="Share document" aria-haspopup="dialog">
            <i class="fas fa-share-alt text-slate-400" aria-hidden="true"></i>
        ...
        <div class="flex items-center rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm"
            ...
            <button type="button"
                    class="w-8 h-8 flex items-center justify-center transition-colors focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500"
                    :class="viewMode === 'grid'
                        ? 'bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400'
                        : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 bg-white dark:bg-slate-800'"
                    @click="viewMode = 'grid'"
                    aria-label="Grid view" :aria-pressed="(viewMode === 'grid').toString()">
                <i class="fas fa-th-large text-[0.7rem]" aria-hidden="true"></i>
            </button>
            <button type="button"
                    class="w-8 h-8 flex items-center justify-center border-l border-slate-200 dark:border-slate-700 transition-colors focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500"
                    :class="viewMode === 'list'
                        ? 'bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400'
                        : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 bg-white dark:bg-slate-800'"
                    @click="viewMode = 'list'"
                    aria-label="List view" :aria-pressed="(viewMode === 'list').toString()">
        ...
        <button type="button"
                class="lg:hidden w-8 h-8 flex items-center justify-center rounded-lg border
                       border-slate-200 dark:border-slate-700 text-slate-400
                       hover:text-indigo-600 dark:hover:text-indigo-400
                       hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors
                       focus:outline-none focus:ring-2 focus:ring-indigo-500"
                @click="detailVisible = !detailVisible"
                :aria-pressed="detailVisible.toString()"
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <aside id="documentProperty"
               class="w-72 xl:w-80 shrink-0 border-l border-slate-200 dark:border-slate-800
                      overflow-y-auto bg-white dark:bg-slate-900
                      transition-all duration-300
                      hidden lg:block"
               :class="detailVisible ? '!block' : ''"
...
<style>
    ...
    #documentContent::-webkit-scrollbar { width: 6px; }
    #documentContent::-webkit-scrollbar-track { background: transparent; }
    #documentContent::-webkit-scrollbar-thumb { background-color: #cbd5e1; border-radius: 3px; }
    .dark-mode #documentContent::-webkit-scrollbar-thumb { background-color: #334155; }
</style>
