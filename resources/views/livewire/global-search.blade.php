<div class="header-search-wrap" wire:keydown.escape="close" @click.outside="$wire.close()">
    ...
    <input type="search" wire:model.live.debounce.300ms="query" placeholder="Search documents, folders, tags…"
        autocomplete="off" @focus="$wire.executeSearch()" />
    ...
    @if($isOpen && count($results))
        <div style="
            ...
            @foreach($grouped as $type => $items)
                <div
                    style="padding:0.4rem 0.9rem 0.15rem;font-size:0.65rem;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:0.08em">
                    {{ ucfirst($type) }}s
                </div>
                @foreach($items as $result)
                    <div wire:click="selectResult('{{ $result['url'] }}', {{ $result['folder_id'] ?? 'null' }})"
                        ...
                        <div
                            style="width:30px;height:30px;border-radius:8px;display:flex;align-items:center;justify-content:center;background:#f1f5f9;flex-shrink:0">
                            <i class="fas {{ $result['icon'] }} {{ $result['color'] }}" style="font-size:0.85rem"></i>
                        </div>
                        <div style="flex:1;min-width:0">
                            <div
                                style="font-size:0.85rem;font-weight:600;color:#1e293b;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
                                {{ $result['label'] }}</div>
