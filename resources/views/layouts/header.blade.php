<header
    class="sticky top-0 z-40 flex w-full min-h-[64px] shrink-0 items-center justify-between gap-x-4 border-b border-[var(--color-border-subtle)] bg-[var(--color-surface)]/60 supports-backdrop-blur:bg-[var(--color-surface)]/60 backdrop-blur-md px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8 dark:bg-[var(--color-surface-dark)]/60 dark:border-[var(--color-border-subtle-dark)] transition-colors pt-[env(safe-area-inset-top)]"
    role="banner">

    <div class="flex items-center gap-4">
        {{-- ── Sidebar Toggle ─────────────────────────── --}}
        <button type="button"
            class="-m-2.5 p-2.5 min-h-[44px] min-w-[44px] flex items-center justify-center text-slate-700 dark:text-slate-300 lg:hidden hover:bg-slate-100 dark:hover:bg-slate-800 rounded-md transition-colors"
        ...
        <div class="lg:hidden flex items-center" aria-hidden="true">
            <a href="{{ route('home') }}" tabindex="-1" wire:navigate
                class="flex items-center gap-2 text-[var(--color-primary)] dark:text-[var(--color-primary-hover)] no-underline">
                <img src="/img/fayiloli-icon.svg" alt="" aria-hidden="true"
                    class="w-8 h-8 shrink-0 rounded-lg shadow-sm">
                <span
                    class="font-bold text-lg tracking-tight text-[var(--color-text-main)] dark:text-[var(--color-text-main-dark)]">
    ...
    <div class="flex items-center gap-x-2 sm:gap-x-4 lg:gap-x-6" role="toolbar" aria-label="Header actions">
        ...
        <div class="flex items-center gap-1 sm:gap-2">
            {{-- Dark mode toggle --}}
            <button type="button"
                class="min-w-[44px] min-h-[44px] flex items-center justify-center rounded-full text-slate-400 hover:text-[var(--color-primary)] dark:hover:text-[var(--color-primary-hover)] hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors focus:ring-2 focus:ring-[var(--color-primary)] focus:outline-none"
                id="darkModeToggle" aria-label="Toggle dark mode" aria-pressed="false"
                onclick="edmsDarkModeToggle(this)">
                <i class="fas fa-moon text-lg" id="darkModeIcon" aria-hidden="true"></i>
            </button>
            ...
            <x-ts-dropdown position="bottom-end">
                <x-slot:action>
                    ...
                    <button type="button" class="flex items-center min-h-[44px] gap-2 pl-1.5 pr-2.5 py-1.5 rounded-lg
                                           ...
                                           hover:border-[var(--color-primary)] dark:hover:border-slate-500
                ...
                <a class="group flex flex-row min-h-[44px] items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors no-underline"
                    role="menuitem" href="{{ $portalUrl }}">
                    <div class="flex items-center justify-center w-8 h-8 rounded-full bg-[var(--color-primary)]/10 dark:bg-[var(--color-primary)]/20 text-[var(--color-primary)] dark:text-[var(--color-primary-hover)] shrink-0 group-hover:scale-105 group-hover:bg-[var(--color-primary)]/20 transition-all"
                        aria-hidden="true">
                        <i class="fas fa-arrow-right-arrow-left"></i>
                    </div>
                    <div class="flex flex-col items-start justify-center">
                        <div
                            class="text-[0.85rem] font-bold text-[var(--color-primary)] dark:text-[var(--color-primary-hover)] leading-none">
                            Central
                            Portal</div>
                        ...
                        @foreach($availableTenants as $t)
                            ...
                            <a href="{{ $switchUrl }}"
                                ...
                                <div class="flex flex-col items-start justify-center overflow-hidden w-full">
                                    <div
                                        class="text-[0.8rem] font-semibold text-slate-700 dark:text-slate-200
        ...
        <x-ts-dropdown position="bottom-end">
            <x-slot:action>
                <button type="button"
                    class="flex items-center gap-x-3 p-1 rounded-full min-h-[44px] hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors focus:ring-2 focus:ring-[var(--color-primary)] focus:outline-none"
                    aria-label="User menu for {{ Auth::user()?->name }}">
                    <span class="hidden lg:flex lg:items-center">
                        <span
                            class="text-sm font-bold leading-6 text-[var(--color-text-main)] dark:text-[var(--color-text-main-dark)]"
                            aria-hidden="true">{{ Auth::user()?->name }}</span>
                    </span>
                    <div class="flex items-center justify-center w-10 h-10 shadow-md rounded-full text-xs font-bold text-[var(--color-primary)] bg-[var(--color-primary)]/10 shrink-0"
                        aria-hidden="true">
